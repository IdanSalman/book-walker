    ...
    <div className="space-y-6 rounded-xl border border-zinc-800 bg-zinc-900/50 p-5">
      <div className="space-y-2">
        <div className="flex items-center justify-between text-sm text-zinc-400">
          <span>Progress</span>
          <span>
            {progressLabel(currentPage, book)}
      ...
      <form action={formAction} className="space-y-5">
        ...
        <div className="flex flex-wrap gap-3">
          {isReadableComic(book.category) && (
            <Link
              href={`/read/${book.id}`}
              className={buttonVariants()}
            >
              <BookOpen className="h-4 w-4" />
              {userBook.currentPage > 0 ? "Continue reading" : "Read"}
            </Link>
          )}
          <Button type="submit" disabled={pending} variant={isReadableComic(book.category) ? "secondary" : "default"}>
